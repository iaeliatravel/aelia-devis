...
export default function DashboardPage() {
  ...
  /* ── Filtre combiné : catégorie + statut + recherche ── */
  const filtered = quotes.filter(q =>
    ...
    (!search ||
      ...
      (q.client?.phone || '').includes(search)   // ← recherche par téléphone
    )
  )

  /* ── Stats par type (acceptés seulement) ── */
  ...
  return (
    <div style={{ padding:mobile?'1rem':'2rem', maxWidth:1200, margin:'0 auto' }} className="animate-fadeIn">
      ...
      {/* Header — SANS bouton Paramètres ici, il est dans ServiceNav */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start',
        ...
        <Link href="/devis/nouveau"
          style={{ padding:'0.5rem 1.25rem', borderRadius:'0.75rem', background:'var(--color-primary)',
      ...
      <div style={{ display:'grid', gridTemplateColumns:mobile?'1fr 1fr':'1fr 1fr 1fr',
        ...
        {[
          ...
          {icon:'📈', val:`+${fmt(totalPrf)} DA`, label:'Bénéfice (acceptés)', green:true},
          {icon:'👥', val:String(totalCli),        label:'Clients'},
        ].map((k, i) => (
          <div key={i} style={{ padding:'0.875rem 1rem', borderRadius:'0.875rem',
            ...
            <div>
              <div style={{ fontSize:mobile?'0.875rem':'1rem', fontWeight:700,
                ...
                color:k.green?'var(--color-success)':undefined }}>{k.val}</div>
              <div style={{ fontSize:'0.6875rem', color:'var(--color-text-muted)', marginTop:1 }}>{k.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Recherche */}
      <input type="text" placeholder="🔍 N°, nom ou téléphone client…" value={search}
        onChange={e => setSearch(e.target.value)}
        style={{ width:'100%', padding:'0.625rem 0.875rem', borderRadius:'0.625rem',
          fontSize:'0.875rem', outline:'none', marginBottom:'0.625rem', ...S }}/>

      {/* Filtres catégorie */}
      <div style={{ display:'flex', gap:4, overflowX:'auto', marginBottom:'0.5rem',
        msOverflowStyle:'none', scrollbarWidth:'none' }}>
        {DOC_FILTERS.map(f => (
          <button key={f.value} onClick={() => setFilter(f.value)}
            style={{ padding:'0.4rem 0.75rem', borderRadius:'0.5rem', fontSize:'0.8125rem',
      ...
      {/* Filtres statut */}
      <div style={{ display:'flex', gap:4, overflowX:'auto', marginBottom:'1rem',
        msOverflowStyle:'none', scrollbarWidth:'none' }}>
        {STATUS_FILTERS.map(f => {
          const s = f.value !== 'all' ? SL[f.value] : null
          const active = statusFilter === f.value
          return (
            <button key={f.value} onClick={() => setStatusFilter(f.value)}
              style={{ padding:'0.4rem 0.75rem', borderRadius:'0.5rem', fontSize:'0.75rem',
                fontWeight:active?700:500, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0,
                background: active ? (s?.bg || 'var(--color-primary)') : 'var(--color-surface)',
                color:       active ? (s?.color || 'white')             : 'var(--color-text-muted)',
      ...
      {loading ? (
        <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
          {[...Array(4)].map((_,i) => <div key={i} className="skeleton" style={{ height:mobile?96:52, borderRadius:'0.875rem' }}/>)}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center',
          ...
          <Link href="/devis/nouveau" style={{ padding:'0.5rem 1.25rem', borderRadius:'0.625rem',
            ...
            fontWeight:600, fontSize:'0.875rem' }}>＋ Nouveau</Link>
        </div>
      ) : mobile ? (
        /* ── MOBILE : cartes ── */
        <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
              ...
              {filtered.map(q => {
                ...
                return (
                  <tr key={q.id} style={{ borderBottom:'1px solid var(--color-border)', transition:'background 0.1s' }}
                    ...
                    <td style={{ padding:'0.75rem 1rem', fontSize:'0.875rem', fontWeight:700,
                      color:'var(--color-primary)', whiteSpace:'nowrap',
                      fontVariantNumeric:'tabular-nums' }}>{q.quote_number}</td>
                    <td style={{ padding:'0.75rem 1rem' }}><DocTypeBadge type={q.document_type} size="sm"/></td>
                    <td style={{ padding:'0.75rem 1rem' }}>
                      {/* Nom + téléphone (plus petit) */}
                      <div style={{ fontSize:'0.875rem', fontWeight:500 }}>{name}</div>
                      {q.client?.phone && (
                        <div style={{ fontSize:'0.75rem', color:'var(--color-text-muted)',
                          marginTop:2, fontVariantNumeric:'tabular-nums' }}>{q.client.phone}</div>
                      )}
                    </td>
                    ...
                    <td style={{ padding:'0.75rem 1rem' }}>
                      <span style={{ padding:'2px 10px', borderRadius:9999, fontSize:'0.6875rem',
                        fontWeight:600, background:s.bg, color:s.color, whiteSpace:'nowrap' }}>{s.label}</span>
                    </td>
                    <td style={{ padding:'0.75rem 1rem', fontSize:'0.875rem', fontWeight:700,
                    ...
                    <td style={{ padding:'0.75rem 1rem', fontSize:'0.875rem', fontWeight:600,
                      ...
                      +{fmt(bp)} DA
                      {hasManual && <span style={{ marginLeft:4, fontSize:'0.6875rem', color:'#f59e0b' }} title="Bénéfice manuel">✎</span>}
                    </td>
                    <td style={{ padding:'0.75rem 1rem' }}>
