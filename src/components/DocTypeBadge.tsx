...
export default function DocTypeBadge({ type, size = 'sm', showIcon = true }: Props) {
  const key = (type as DocumentType) || 'devis'
  const cfg = DOC_TYPE_CONFIG[key] ?? DOC_TYPE_CONFIG.devis

  const pad  = size === 'sm' ? '2px 8px'  : '4px 12px'
  const font = size === 'sm' ? '0.7rem'   : '0.8125rem'
  ...
  return (
    <span style={{
      ...
      {showIcon && <span style={{ fontSize: size === 'sm' ? '0.7rem' : '0.875rem' }}>{cfg.icon}</span>}
      {cfg.label}
    </span>
  )
}
