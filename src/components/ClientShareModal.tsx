...
export default function ClientShareModal({ quote, agency, onClose, onDocTypeChange }: Props) {
  ...
  function buildBaseHTML(mode: GenMode, dt: 'devis'|'proforma'|'facture'): string {
    ...
    const metaItems = [
      { label:'N° Document', val: esc(quote.quote_number) },
    ...
    const cachetHTML = addCachet ? `
      <div style="width:${fs(80,100)}px;height:${fs(80,100)}px;border:2px solid #0d2340;border-radius:50%;
        display:flex;flex-direction:column;align-items:center;justify-content:center;
        text-align:center;padding:${fs(6,8)}px;opacity:0.65;flex-shrink:0">
        <div style="font-size:${fs(6.5,8)}px;font-weight:800;text-transform:uppercase;
          letter-spacing:.04em;color:#0d2340;line-height:1.3">${esc(agency.name)}</div>
        <div style="width:55%;height:1px;background:#0d2340;margin:${fs(2,3)}px 0;opacity:0.35"></div>
        <div style="font-size:${fs(5.5,7)}px;color:#0d2340;opacity:0.75">Agence de voyages</div>
      </div>` : ''

    return `<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"/><title>${word} ${esc(quote.quote_number)}</title>
<style>
...
<div style="background:#fff;${p?'':'border-radius:6px;overflow:hidden;margin:24px auto;max-width:1040px;box-shadow:0 4px 32px rgba(0,0,0,.1)'}">
  ...
  <div style="padding:${p?'18px 34px 28px':'24px 48px 40px'}">
    ...
    <div style="display:flex;justify-content:flex-end;align-items:center;gap:${fs(12,16)}px;margin-bottom:${fs(10,14)}px">
      ...
      <div style="-webkit-print-color-adjust:exact;print-color-adjust:exact;
        background:#0d2340;border-radius:8px;padding:${p?'12px 18px':'16px 24px'};
        text-align:right;min-width:${fs(185,240)}px">
  ...
  function buildBonHTML(mode: GenMode): string {
    ...
    const cachetBonHTML = addCachet ? `
      <div style="width:${fs(74,92)}px;height:${fs(74,92)}px;border:2px solid #F0997B;border-radius:50%;
        display:flex;flex-direction:column;align-items:center;justify-content:center;
        text-align:center;padding:${fs(5,7)}px;opacity:0.7;flex-shrink:0">
        <div style="font-size:${fs(6,7.5)}px;font-weight:800;text-transform:uppercase;
          letter-spacing:.04em;color:#D85A30;line-height:1.3">${esc(agency.name)}</div>
        <div style="width:55%;height:1px;background:#D85A30;margin:${fs(2,3)}px 0;opacity:0.35"></div>
        <div style="font-size:${fs(5.5,6.5)}px;color:#D85A30;opacity:0.8">Agence de voyages</div>
      </div>` : ''
    ...
    <div style="position:relative;background:#fff;border:.75px solid #d3d1c7;
      ...
      <div style="-webkit-print-color-adjust:exact;print-color-adjust:exact;
        ...
        <div style="margin-left:auto;text-align:right">
          ...
          <div style="font-size:${fs(9,11)}px;color:rgba(255,255,255,.45);margin-top:3px">N° ${esc(quote.quote_number)}</div>
        </div>
      </div>
      ...
      <div style="-webkit-print-color-adjust:exact;print-color-adjust:exact;
        background:#faf8f4;border-bottom:1px solid #f3f0e8;
      ...
      <div style="display:flex;align-items:stretch;gap:${fs(8,12)}px;
        padding:${fs(8,12)}px ${fs(18,24)}px ${fs(10,14)}px">
        <div style="flex:1.5;background:#faf8f4;border-radius:6px;padding:${fs(7,10)}px;
          border:1px solid #D3D1C7;min-height:${fs(44,56)}px">
    ...
    return `<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"/><title>Bon de Versement ${esc(quote.quote_number)}</title>
...
<body><div style="${p?'':'padding:24px;max-width:1080px;margin:0 auto'}">${body}</div></body></html>`
  ...
  /* ── Image 1080px — via iframe pour rendu correct ── */
  async function generateImage() {
    ...
    try {
      ...
      iDoc.open()
      iDoc.write(getHTML('image'))
      iDoc.close()

      // Attendre le rendu complet + images
      await new Promise(r => setTimeout(r, 1800))

      const target = (
        iDoc.querySelector('.doc') ||
        iDoc.querySelector('[style*="border-radius:6px"]') ||
        iDoc.querySelector('[style*="border-radius: 6px"]') ||
      ...
      link.download = `${currentTab.label.replace(/\s+/g,'_')}_${quote.quote_number.replace('/','_')}.jpg`
        ...
        <div style={{padding:'1.25rem',display:'flex',flexDirection:'column',gap:'0.875rem'}}>
          ...
          <div>
            <div style={{fontSize:'0.7rem',fontWeight:600,textTransform:'uppercase',
              letterSpacing:'0.08em',color:'#6b7280',marginBottom:'0.5rem'}}>Type de document</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
              {DOC_TABS.map(tab => {
                const tc = DOC_TYPE_CONFIG[tab.dbKey]
            ...
            {isChanged && (
              <button onClick={saveDocType} disabled={saving}
                ...
                {saving ? '⏳ Sauvegarde...' : saved ? `✅ Converti en "${currentTab.label}"` : `💾 Sauvegarder → "${currentTab.label}"`}
              </button>
            )}
          </div>
          ...
          <label style={{display:'flex',alignItems:'center',gap:'0.625rem',cursor:'pointer',
            ...
            <div>
              <div style={{fontSize:'0.8125rem',fontWeight:600,color:addCachet?'#0d2340':'#374151'}}>
                🔵 Ajouter le cachet agence
              </div>
              <div style={{fontSize:'0.75rem',color:'#6b7280'}}>
                Affiché à gauche du total dans le document
              </div>
            </div>
          </label>
          ...
          <div style={{background:logoOk?'#f0fdf4':'#fffbeb',border:`1px solid ${logoOk?'#bbf7d0':'#fde68a'}`,
            ...
            {logoOk
              ? '✅ Logo configuré — visible dans le document'
