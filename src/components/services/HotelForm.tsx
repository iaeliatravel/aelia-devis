...
const BOARD_OPTIONS = [
  ...
  { value: 'half_board_plus', label: 'Demi pension plus' },
...
function grid(cols: number): React.CSSProperties {
  return { display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '1rem' }
}
...
export default function HotelForm({ data, onChange }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={grid(3)}>
        <AutocompleteInput label="Nom de l'hôtel *" category="hotel"
          value={data.hotel_name||''} onChange={v => set('hotel_name', v)}
          placeholder="Ex: Abou Sofiane Hotel" required />
        <AutocompleteInput label="Ville *" category="city"
          value={data.hotel_city||''} onChange={v => set('hotel_city', v)}
          placeholder="Ex: Sousse" required />
        <AutocompleteInput label="Pays *" category="country"
          value={data.hotel_country||''} onChange={v => set('hotel_country', v)}
          placeholder="Ex: Tunisie" required />
      </div>
      ...
      <div style={grid(4)}>
        ...
        <SelectField label="Type de chambre *"
          value={data.room_type||''} onChange={v => set('room_type', v)}
          options={ROOM_OPTIONS} placeholder="Choisir..." required />
        <SelectField label="Hébergement *"
          value={data.board_type||''} onChange={v => set('board_type', v)}
          options={BOARD_OPTIONS} placeholder="Choisir..." required />
      </div>

      <div style={grid(3)}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={L}>Date d&apos;arrivée *</label>
          <input type="date" value={data.checkin_date||''}
            onChange={e => set('checkin_date', e.target.value)} required style={inp} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={L}>Date de départ *</label>
          <input type="date" value={data.checkout_date||''}
            min={data.checkin_date || undefined}
            onChange={e => set('checkout_date', e.target.value)} required style={inp} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={L}>Nuitées (auto)</label>
          <div style={{ padding: '0.5rem 0.75rem', borderRadius: '0.5rem',
            border: '1px solid var(--color-border)', background: 'var(--color-surface-offset)',
            fontWeight: 600, color: 'var(--color-primary)', fontSize: '0.875rem' }}>
